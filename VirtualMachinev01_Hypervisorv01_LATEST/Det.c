...
void infSwcServiceDetSwcApplEcuM_vInitFunction(const Type_CfgSwcServiceDet_st* ConfigPtr){
...
Std_ReturnType Det_ReportError(
   ...
   if(!lbInitialized){
      ...
      if(Det_BufferIndex_u16 < (DET_CFG_ERRORBUFFERSIZE)){
         ...
         for(
            errorCount = 0;
            errorCount < Det_BufferIndex_u16;
            errorCount ++
         ){
            if(
                  (Det_ErrorEntryBuffer[errorCount].ModuleId   == ModuleId)
...
Std_ReturnType Det_ReportTransientFault(uint16 ModuleId, uint8 InstanceId, uint8 ApiId, uint8 FaultId){
   ...
   if(!lbInitialized){
      ...
      retval = DET_CFG_CALLTRANSIENTCALLOUT(
            ModuleId
         ,  InstanceId
         ,  ApiId
         ,  FaultId
